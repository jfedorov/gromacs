...
and :mdp:`awh1-share-group` (for bias 1) to a non-zero value. Then bias 1
will be shared between simulations that have the same value for the share
group. Sharing can be different for bias 1, 2, etc. (although there are
...
Note that biases can not be shared within a simulations.
