...
{
    ListedForceCalculator nblibCalculator(
            nblibData.interactions, nblibData.x.size(), numThreads, *nblibData.box);
    ...
    // 2nd pass to make sure that the force output stays the same for identical input
    nblibForces = std::vector<gmx::RVec>(nblibData.x.size(), Vec3{ 0, 0, 0 });
    nblibCalculator.compute(nblibData.x, nblibForces, nblibEnergies);

    ListedForceCalculator::EnergyType gmxEnergies;
    std::vector<gmx::RVec>            gmxForces(gmxData.x.size(), Vec3{ 0, 0, 0 });
    gmxCalculator.compute(gmxData.x, gmxForces, gmxEnergies, false);
...
{
public:
    listedBenchmark(DataObject data, int reps_, int nThr) :
        data_(std::move(data)),
        numThreads(nThr),
        reps(reps_),
    ...
    void operator()()
    {
        ...
        {
            lfCalculator.compute(data_.x, forceBuffer_, calculatorEnergies, false);
        }
    }
...
int main()
{
    ...
    // verify correctness of the forces
