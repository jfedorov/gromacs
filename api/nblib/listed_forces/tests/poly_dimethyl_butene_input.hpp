...
public:
    ...
    , bondLength(0.01)
    {
        ...
        box.reset(new Box{3 * a * length, 1, 1});
    }
...
private:
    ...
    void setInteractionParameters()
    {
        ...
        {
            auto& p = pickType<HarmonicBondType>(interactions).parameters;
            p.insert(begin(p), begin(bondsp), end(bondsp));
        }
        {
            auto& p = pickType<HarmonicAngle>(interactions).parameters;
            p.insert(begin(p), begin(anglesp), end(anglesp));
        }
        {
            auto& p = pickType<ProperDihedral>(interactions).parameters;
