...
public:
    ...
    {
        std::string integrator("md");

        gmx::test::CommandLine caller;
        runner_.useTopGroAndNdxFromDatabase(filename);
        runner_.useStringAsMdpFile(gmx::test::prepareMdpFileContents(
                gmx::test::prepareMdpFieldValues(filename.c_str(), integrator.c_str(), "no", "no")));
...
TEST_F(TprReaderTest, FCfromTprDataWorks)
{
    ...
    auto forceCalculatorTpr = GmxSetupDirector::setupGmxForceCalculatorCpu(tprReader, options);
