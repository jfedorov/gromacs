...
public:
    ...
    [[maybe_unused]] static void checkReferenceImplementation()
    {
        ...
        for (const auto& vSite : virtualSites)
        {
            SCOPED_TRACE("Checking virtual site " + vSiteTypeToString(vSite.type));
            ...
            GMX_RELEASE_ASSERT(std::abs(displacement1[XX]) > 0 && std::abs(displacement1[YY]) > 0
                                       && std::abs(displacement1[ZZ]) > 0,
                               "Zero displacement - adjust choice of coordinates or time step.");
    ...
    struct VirtualSite
    {
        ...
        int atomIdx;
        //! Index of constructing atoms
        std::vector<int> constructingAtomIdx;
        //! Construction parameters
        std::vector<real> parameters;

        //! Returns position and velocity of virtual site
        [[nodiscard]] std::tuple<RVec, RVec> calculate(ArrayRef<const RVec> positions,
    ...
    //! Returns the required parameters from topology
    static std::vector<real> getVSiteParameters(const t_iparams& params, int vSiteType)
    {
        switch (vSiteType)
        {
            case F_VSITE1: return {};
            case F_VSITE2:
            case F_VSITE2FD: return { params.vsite.a };
            case F_VSITE3:
            case F_VSITE3FD:
            case F_VSITE3FAD: return { params.vsite.a, params.vsite.b };
            case F_VSITE3OUT:
            case F_VSITE4FDN: return { params.vsite.a, params.vsite.b, params.vsite.c };
            default: throw NotImplementedError("Unknown virtual site type");
        }
    }

    //! Helper function return the virtual site type as string
    static std::string vSiteTypeToString(int vSiteType)
    {
        switch (vSiteType)
        {
            case F_VSITE1: return "1";
            case F_VSITE2: return "2";
            case F_VSITE2FD: return "2FD";
            case F_VSITE3: return "3";
            case F_VSITE3FD: return "3FD";
            case F_VSITE3FAD: return "3FAD";
            case F_VSITE3OUT: return "3OUT";
            case F_VSITE4FD: return "4FD";
            case F_VSITE4FDN: return "4FDN";
            case F_VSITEN: return "N";
            default: throw NotImplementedError("Unknown virtual site type");
        }
    }
    ...
    static std::vector<VirtualSite> vSiteList(const TopologyInformation& topologyInformation)
    {
        ...
        for (int vsiteType = F_VSITE1; vsiteType <= F_VSITEN; vsiteType++)
        {
            ...
            if (vsiteType == F_VSITE4FD || interactionList.empty())
            {
                ...
                printf("  * VSite type %4s: not present\n", vSiteTypeToString(vsiteType).c_str());
                continue;
            }
            ...
            for (int i = 0; i < interactionList.size();)
            {
                ...
                else
                {
                    virtualSites.emplace_back(VirtualSite{
                            ...
                            getVSiteParameters(localTopology.idef.iparams[parameterIdx], vsiteType) });
                    i += defaultIncrement;
                }
            }
            printf("  * VSite type %4s: Index { %s }\n",
                   vSiteTypeToString(vsiteType).c_str(),
                   indexString.c_str());
        }
        return virtualSites;
                                                         ...
                                                         ArrayRef<const RVec> velocities) const
{
    ...
    const auto f3fd = /*(1 - a) **/ rij + a * rjk;
    const auto v3fd = /*(1 - a) **/ vij + a * vjk;

    return { ri + (b / f3fd.norm()) * f3fd,
             vi + (b / f3fd.norm()) * (v3fd - f3fd * (v3fd.dot(f3fd) / f3fd.norm2())) };
}
template<>
[[nodiscard]] std::tuple<RVec, RVec>
...
[[nodiscard]] std::tuple<RVec, RVec> VirtualSiteTest::VirtualSite::calculate(ArrayRef<const RVec> positions,
                                                                             ArrayRef<const RVec> velocities) const
{
    switch (type)
    {
        case F_VSITE1: return calculateVSite<F_VSITE1>(positions, velocities);
        case F_VSITE2: return calculateVSite<F_VSITE2>(positions, velocities);
        case F_VSITE2FD: return calculateVSite<F_VSITE2FD>(positions, velocities);
        case F_VSITE3: return calculateVSite<F_VSITE3>(positions, velocities);
        case F_VSITE3FD: return calculateVSite<F_VSITE3FD>(positions, velocities);
        case F_VSITE3FAD: return calculateVSite<F_VSITE3FAD>(positions, velocities);
        case F_VSITE3OUT: return calculateVSite<F_VSITE3OUT>(positions, velocities);
        case F_VSITE4FDN: return calculateVSite<F_VSITE4FDN>(positions, velocities);
        case F_VSITEN: return calculateVSite<F_VSITEN>(positions, velocities);
        default: throw NotImplementedError("Unknown virtual site type");
    }
}
//! \endcond
...
TEST_P(VirtualSiteTest, WithinToleranceOfReference)
{
    ...
    auto tolerance = defaultRealTolerance();
    if (integrator == "bd")
    {
        // Default real tolerance is 4 ULP.
        // BD needs a larger tolerance
        // TODO: Justify - I don't see any code problems, but I don't
        //       really understand why we need a larger tolerance
        tolerance = relativeToleranceAsUlp(1.0, 100);
    }
