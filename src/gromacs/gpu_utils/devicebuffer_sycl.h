...
public:
    ...
    ClSyclBufferWrapper(T* ptr, cl::sycl::context context) : ptr_(ptr), context_(context) {}
...
public:
    ...
    __attribute__((always_inline)) T* get_pointer() const noexcept { return ptr_; }
    __attribute__((always_inline)) T& operator[](cl::sycl::id<1> index) const
    {
        return ptr_[index.get(0)];
    }
    __attribute__((always_inline)) T& operator[](size_t index) const { return ptr_[index]; }
...
#if GMX_SYCL_USE_USM
    deviceStream.stream().submit([&](cl::sycl::handler& cgh) {
