...
void gpu_launch_kernel_pruneonly(NbnxmGpu* nb, const gmx::InteractionLocality iloc, const int numParts)
{
    ...
    if (bDoTime)
    {
        timer = &(plist->haveFreshList ? nb->timers->interaction[iloc].prune_k
                                       : nb->timers->interaction[iloc].rollingPrune_k);
        timer->openTimingRegion(deviceStream);
    }

    auto* timingEvent = bDoTime ? timer->fetchNextEvent() : nullptr;

    launchNbnxmKernelPruneOnly(nb, iloc, numParts, part, numSciInPart, timingEvent);
