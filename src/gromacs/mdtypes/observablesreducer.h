 ...
 * done rarely and only when required.
 *
 ...
 * if all active modules required communication this step. This
 * eliminates the need for copying data, if all modules are set up in
 * advance with a stable view of memory into which they can store data
 * for which reduction is desired. It also means that modules not
 * active in the current simulation do not contribute to the workload
 * at run time. Also, modules that are active but don't need
 * communication at any particular MD step can passively opt out, ie
 * does not require branching or function calls.
 *
...
\endmsc
 *
 * Three callbacks are used per participating module:
 *
 * 1. one made by the module so it can be notified when reduction is complete.
 * 2. one made by the builder and returned to the module so the latter can
 *    require reduction with the first callback when it wishes
 * 3. one made by the module and passed to the builder so the latter can
 *    notify the former of the buffer view it should use and receive a
 *    copy of the second callback.
 *
 * Modules are able to request reduction to occur whenever some other
 * module requires it, so the total number of reductions is kept
 * low. Naturally, the callback to such a module happens only after
 * the eventual reduction, which may happen on the same step or a
 * later one.
 *
 ...
 * likely change into a doReduction() method, or similar.
 *
 * \inlibraryapi
...
enum class ReductionRequirement : int
{
    ...
    Soon,
    /*! \brief Reduce whenever the runner next checks with the ObservablesReducer
     * after some module requires reduction Soon */
    Eventually
};
 ...
 * - only registered subscribers can require reduction,
 * - the ObservablesReducer::Impl has a stable address from the
 *   heap allocation needed for std::unique_ptr.
 *
 ...
 * orchestating communication based on the current step number or a
 * previous message.
 *
...
public:
    ...
    using CallbackToRequireReduction = std::function<void(ReductionRequirement)>;
    ...
    /*! \brief Build a \c ObservablesReducer to which any subscribers have been added
     *
     * Must be called only once. Notifies subscribers via the
     * CallbackFromBuilder that they supplied. */
 ...
 * they have subscribed do communication this step and provide a
 * callback function so they can react to the reduced values. It
 * is the responsibility of subscribed modules to ensure that this
 * behaviour is fully consistent across MPI ranks.
 *
 * After reduction is complete and notifications made, buffer is
 * zeroed. The subscribers may not depend on the values in their
 * buffer view after the notification callback returns, so should do
 * any necessary processing during that callback.
 *
...
public:
    ...
    ObservablesReducer();
    //! Constructor only used in testing
    explicit ObservablesReducer(std::vector<double>&& communicationBuffer,
    ...
    int registerCallback(ObservablesReducerBuilder::CallbackAfterReduction&& callback);
     ...
     * until they overflow or underflow.
     *
     ...
     * inconsistent use by subscribers. However by construction and
     * expectation, the integration tests never produce a zero result
