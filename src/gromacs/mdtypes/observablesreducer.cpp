...
/*! \internal \file
 * \brief
 * Defines gmx::ObservablesReducer and builder
 *
 * \inlibraryapi
...
public:
     ...
     */
    void requireReduction(int callbackIndex, ReductionRequirement requirement)
    {
    ...
    void reset()
    {
        std::fill(communicationBuffer_.begin(), communicationBuffer_.end(), 0.0);
        callbacksAfterReduction_.clear();
        reduceSoon_ = false;
    }

    //! Storage for communication buffer, must never be resized
    ...
    //! Indices into registeredCallbacks_ of callbacks to use after communication this step
...
ObservablesReducer::ObservablesReducer() : impl_(std::make_unique<Impl>()) {}

ObservablesReducer::ObservablesReducer(std::vector<double>&& communicationBuffer,
                                       std::vector<ObservablesReducerBuilder::CallbackAfterReduction>&& registeredCallbacks) :
    impl_(std::make_unique<Impl>(std::move(communicationBuffer), std::move(registeredCallbacks)))
{
}
    ...
    for (const Impl::Subscription& subscription : impl_->subscriptions_)
    {
        ...
        CallbackToRequireReduction callbackToRequireReduction =
                [impl, indexToCallbackAfterReduction](ReductionRequirement requirement) {
                    impl->requireReduction(indexToCallbackAfterReduction, requirement);
                };
