                                               ...
                                               gmx_wallcycle_t        wcycle)
{
    if (pme_gpu_settings(pme->gpu).performGPUFFT)
    {
        ...
        wallcycle_sub_start_nocount(wcycle, WallCycleSubCounter::LAUNCH_GPU_PME);
        pme_gpu_3dfft(pme->gpu, dir, gridIndex);
        wallcycle_sub_stop(wcycle, WallCycleSubCounter::LAUNCH_GPU_PME);
        wallcycle_stop(wcycle, WallCycleCounter::LAUNCH_GPU);
    }
    else
    {
        wallcycle_start(wcycle, WallCycleCounter::PME_FFT_MIXED_MODE);
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
        ...
        wallcycle_stop(wcycle, WallCycleCounter::PME_FFT_MIXED_MODE);
    }
}
...
void pme_gpu_launch_complex_transforms(gmx_pme_t* pme, gmx_wallcycle* wcycle, const gmx::StepWorkload& stepWork)
{
    ...
    {
        wallcycle_start(wcycle, WallCycleCounter::WAIT_GPU_PME_SPREAD);
        pme_gpu_sync_spread_grid(pme->gpu);
        wallcycle_stop(wcycle, WallCycleCounter::WAIT_GPU_PME_SPREAD);
    }
    ...
    try
    {
        ...
        for (int gridIndex = 0; gridIndex < pmeGpu->common->ngrids; gridIndex++)
        {
            ...
            {
                wallcycle_start(wcycle, WallCycleCounter::PME_SOLVE_MIXED_MODE);
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
                for (int thread = 0; thread < pme->nthread; thread++)
                {
                    solve_pme_yzx(pme, cfftgrid, pme->boxVolume, computeEnergyAndVirial, pme->nthread, thread);
                }
                wallcycle_stop(wcycle, WallCycleCounter::PME_SOLVE_MIXED_MODE);
            }
...
{
    wallcycle_start(wcycle, WallCycleCounter::PME_GPU_F_REDUCTION);
    ...
    wallcycle_stop(wcycle, WallCycleCounter::PME_GPU_F_REDUCTION);
}
                             ...
                             GpuTaskCompletion        completionKind)
{
    ...
    {
        wallcycle_start_nocount(wcycle, WallCycleCounter::WAIT_GPU_PME_GATHER);
        ...
        wallcycle_stop(wcycle, WallCycleCounter::WAIT_GPU_PME_GATHER);
    ...
    wallcycle_start(wcycle, WallCycleCounter::WAIT_GPU_PME_GATHER);
