                ...
                const gmx::DeviceStreamManager* deviceStreamManager)
{
    ...
    do /****** this is a quasi-loop over time steps! */
    {
        ...
        if (useGpuForPme)
        {
            ...
            if (!pme_pp->useGpuDirectComm)
            {
                // We don't mark the event because we put everything into a single stream
                stateGpu->copyCoordinatesToGpu(
                        gmx::ArrayRef<gmx::RVec>(pme_pp->x), gmx::AtomLocality::Local, false);
            }
