...
#    endif
    ...
    if (ms != nullptr)
    {
        ...
        MPI_Comm_rank(cr->mpiDefaultCommunicator, &cr->nodeIdOfDefaultCommunicator);
#else
    ...
    else
    {
        ...
        cr->nodeIdOfDefaultCommunicator = rankInCommunicator;
    }
    // For now, we want things to go horribly wrong if this is used too early...
    // TODO: Remove when communicators are removed from commrec
    cr->nnodes           = -1;
    cr->nodeid           = -1;
    cr->sim_nodeid       = -1;
