    ...
    boxDeformation_(boxDeformation)
{
    ...
    energyData->addConservedEnergyContribution([this](Step step, Time /*unused*/) {
        ...
        {
            GMX_THROW(ModSimRuntimeError("Parrinello-Rahman conserved energy step mismatch."));
        }
        return conservedEnergyContribution_;
    });
}

void ParrinelloRahmanBarostat::connectWithPropagator(const PropagatorConnection& connectionData,
                                                     const PropagatorTag&        propagatorTag)
{
    if (connectionData.tag == propagatorTag)
    {
        GMX_RELEASE_ASSERT(connectionData.parrinelloRahmanScaling,
