...
// Enum to string
inline static std::string toString(NhcUsage nhcUsage)
{
    switch (nhcUsage)
    {
        case NhcUsage::System: return "System";
        case NhcUsage::Barostat: return "Barostat";
        default: GMX_THROW(NotImplementedError("Unknown NHC Usage"));
    }
}
...
NoseHooverChainsData::NoseHooverChainsData(int         numTemperatureGroups,
                                           ...
                                           NhcUsage    nhcUsage) :
    identifier_(std::string("NoseHooverChainsData-") + toString(nhcUsage)),
            ...
            for (auto chainPosition = 0; chainPosition < chainLength; ++chainPosition)
            {
                ...
                invXiMass_[temperatureGroup][chainPosition] =
                        ...
                        / (gmx::square(couplingTime_[temperatureGroup] / M_2PI)
                           * referenceTemperature_[temperatureGroup] * numDof * BOLTZ);
...
{
    if (coordinateViewInUse_[temperatureGroup])
    {
        GMX_THROW(ModSimRuntimeError("xi view was already requested and not returned."));
    }
...
void NoseHooverChainsData::calculateIntegral(int temperatureGroup)
{
    ...
    for (auto chainPosition = 0; chainPosition < chainLength_; ++chainPosition)
    {
        ...
        integral += 0.5 * gmx::square(xiVelocities_[temperatureGroup][chainPosition])
                            / invXiMass_[temperatureGroup][chainPosition]
                    + numDegreesOfFreedom * xi_[temperatureGroup][chainPosition] * BOLTZ
...
double NoseHooverChainsData::temperatureCouplingIntegral(Time time) const
{
     ...
     * rest of the system.
     */
    if (std::any_of(integralTime_.begin(), integralTime_.end(), [time](real integralTime) {
            return !(time <= integralTime || timesClose(integralTime, time));
        }))
    {
        GMX_THROW(ModSimRuntimeError("NoseHooverChainsData conserved energy time mismatch."));
    }
...
{
    return "NoseHooverChainsData" + toString(nhcUsage);
}
        ...
        for (unsigned int syInnerLoop = 0; syInnerLoop < c_suzukiYoshidaOrder; ++syInnerLoop)
        {
            ...
            for (auto chainPosition = chainLength - 1; chainPosition >= 0; --chainPosition)
            {
                ...
                const real xiAcceleration = invXiMass[chainPosition]
                                            * (kineticEnergy2 - numDof * BOLTZ * referenceTemperature);
            ...
            const real systemScalingFactor = exp(-timeStep * xiVelocities[0]);
...
inline real NoseHooverChainsElement::currentKineticEnergy(const t_grp_tcstat& tcstat)
{
    ...
    {
        GMX_THROW(NotImplementedError("Unknown NhcUsage."));
    }
}
...
{
    if (nhcUsage_ == NhcUsage::System && !propagatorCallback_)
    {
        GMX_THROW(MissingElementConnectionError(
