...
void FirstOrderPressureCoupling::scaleBoxAndCoordinates()
{
    ...
    auto* positions = as_rvec_array(statePropagatorData_->positionsView().unpaddedArrayRef().data());
    rvec* velocities = nullptr;
    if (pressureCouplingType == PressureCoupling::CRescale)
    {
        velocities = as_rvec_array(statePropagatorData_->velocitiesView().unpaddedArrayRef().data());
    }
    // Freeze groups are not implemented
    const unsigned short* cFreeze = nullptr;
