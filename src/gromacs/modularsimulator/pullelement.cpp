...
void PullElement::elementSetup()
{
    ...
    {
        t_pbc pbc;
        set_pbc(&pbc, pbcType_, statePropagatorData_->constBox());
        initPullComFromPrevStep(
                commrec_,
                pullWork_,
                mdAtoms_->mdatoms()->massT,
                &pbc,
                as_rvec_array(statePropagatorData_->constPositionsView().unpaddedArrayRef().data()));
        updatePrevStepPullCom(pullWork_);
    }
}
...
void PullElement::scheduleTask(Step /*unused*/, Time /*unused*/, const RegisterRunFunction& registerRunFunction)
{
    ...
    {
        registerRunFunction([this]() { updatePrevStepPullCom(pullWork_); });
    }
}
