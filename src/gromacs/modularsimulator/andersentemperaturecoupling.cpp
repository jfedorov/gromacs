...
void AndersenTemperatureCoupling::apply(Step step)
{
    ...
    for (int atomIdx = 0; atomIdx < mdAtoms_->homenr; ++atomIdx)
    {
        ...
        bool randomizeAtom = doMassive_;
        if (!doMassive_)
        {
            uniformDist.reset();
            randomizeAtom = uniformDist(rng) < randomizationRate_;
        }

        if (randomizeAtom)
        {
            const real scalingFactor = std::sqrt(BOLTZ * referenceTemperature_[temperatureGroup]
