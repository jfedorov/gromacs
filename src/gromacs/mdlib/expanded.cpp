...
static void simulatedTemperingUpdateTemperature(t_inputrec*      ir,
                                                ...
                                                const int        lamnew)
{
    real*            buf_ngtc = nullptr;
    const t_simtemp* simtemp  = ir->simtempvals;
    ...
    for (int i = 0; i < ir->opts.ngtc; i++)
    {
        ...
        {
            const real told   = ir->opts.ref_t[i];
                             ...
                             const t_mdatoms*      mdatoms)
{
    ...
    {
        simulatedTemperingUpdateTemperature(ir, state, MassQ, v, mdatoms, newLambda);
    }
    return newLambda;
}
