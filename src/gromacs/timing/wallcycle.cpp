...
static void wallcycle_all_start(gmx_wallcycle* wc, WallCycleCounter ewc, gmx_cycles_t cycle)
{
    wc->ewc_prev   = ewc;
    wc->cycle_prev = cycle;
}

static void wallcycle_all_stop(gmx_wallcycle* wc, WallCycleCounter ewc, gmx_cycles_t cycle)
{
...
static void debug_start_check(gmx_wallcycle* wc, WallCycleCounter ewc)
{
    ...
    wc->impl_->counterlist[wc->impl_->count_depth] = ewc;
    wc->impl_->count_depth++;
...
static void debug_stop_check(gmx_wallcycle* wc, WallCycleCounter ewc)
{
    ...
    if (wc->impl_->counterlist[wc->count_depth] != ewc)
    {
        gmx_fatal(FARGS,
                  "wallcycle mismatch at stop, start %s, stop %s",
                  enumValuetoString(wc->impl_->counterlist[wc->impl_->count_depth]),
                  enumValuetoString(ewc));
    }
}
#endif
...
double wallcycle_stop(gmx_wallcycle* wc, WallCycleCounter ewc)
{
     ...
     * large, especially for WallCycleCounter::RUN. If we detect a negative count,
...
{
    return (ewc >= WallCycleCounter::PME_REDISTXF && ewc < WallCycleCounter::PMEWAITCOMM);
}
...
void wallcycle_scale_by_num_threads(gmx_wallcycle* wc, bool isPmeRank, int nthreads_pp, int nthreads_pme)
{
    ...
    if (useCycleSubcounters && !isPmeRank)
    {
        for (auto key : keysOf(wc->wcsc))
        {
            wc->wcsc[key].c *= nthreads_pp;
        }
    }
}
...
WallcycleCounts wallcycle_sum(const t_commrec* cr, gmx_wallcycle* wc)
{
    ...
    subtract_cycles(wcc, WallCycleCounter::PME_FFT, WallCycleCounter::PME_FFTCOMM);
    ...
    if (cr->nnodes > 1)
    {
        ...
        // TODO Use MPI_Reduce
        MPI_Allreduce(
                cyclesMainOnNode.data(), bufMain.data(), sc_numWallCycleCounters, MPI_DOUBLE, MPI_MAX, cr->mpi_comm_mysim);
        if (useCycleSubcounters)
        {
            MPI_Allreduce(cyclesSubOnNode.data(),
                          bufSub.data(),
                          sc_numWallCycleSubCounters,
                          MPI_DOUBLE,
                          MPI_MAX,
                          cr->mpi_comm_mysim);
        }
        ...
        // TODO Use MPI_Reduce
        MPI_Allreduce(
                cyclesMain.data(), cycles_sum.data(), sc_numWallCycleCounters, MPI_DOUBLE, MPI_SUM, cr->mpi_comm_mysim);
        if (useCycleSubcounters)
        {
...
#endif
    {
        for (int i = 0; i < sc_numWallCycleCounters; i++)
        {
            cycles_sum[i] = cyclesMain[i];
        }
        if (useCycleSubcounters)
        {
            for (int i = 0; i < sc_numWallCycleSubCounters; i++)
            {
                const int offset   = i + sc_numWallCycleCounters;
                cycles_sum[offset] = cyclesSub[i];
            }
        }
    }
                     ...
                     const gmx_wallclock_gpu_pme_t*   gpu_pme_t)
{
    ...
    /* When using PME-only nodes, the next line is valid for both
       PP-only and PME-only nodes because they started WallCycleCounter::RUN at the
       same time. */
    ...
    for (auto key = iter.begin() + 4; key != iter.end(); key++)
    {
    ...
    if (wc->wcc[WallCycleCounter::NB_XF_BUF_OPS].n > 0
...
extern int64_t wcycle_get_reset_counters(gmx_wallcycle* wc)
{
    if (wc == nullptr)
    {
        return -1;
...
extern void wcycle_set_reset_counters(gmx_wallcycle* wc, int64_t reset_counters)
{
    if (wc == nullptr)
    {
        return;
